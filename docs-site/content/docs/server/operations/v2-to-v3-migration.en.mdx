...
`source_commit` is currently required and must equal the example commit SHA for original `v2.2.5-20260422`. Missing or different values are rejected. It is source-version evidence supplied by you, not automatic identification of the old binary. Set `shard_count` to the actual original business DB shard count. Target node IDs are 1–1023; replica counts cannot exceed the target node count. `slot_count` sets target logical Slots. The example uses the v3 default of `12`, which must match `cluster.initial_slot_count` at startup. The migration plan still requires an explicit value; no default is filled in. `hash_slot_count` is the physical hash-slot count, fixed at `256`.
