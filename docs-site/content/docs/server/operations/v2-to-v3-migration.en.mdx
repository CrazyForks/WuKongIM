...
On macOS, use `shasum -a 256` instead of `sha256sum`. Build for the actual runtime platform. For a delivery package, compare its archive digest with an independent delivery record before checking the enclosed files. Since v3.0.0-beta.12, official archives and native packages include both binaries; see [wkcli installation](/en/server/tools/wkcli). Compare `wkcli version --output json` with `wukongim version --output json`: version, commit, and build source must match before migration.
