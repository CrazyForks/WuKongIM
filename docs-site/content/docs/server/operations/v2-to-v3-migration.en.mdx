...
`source_commit` is version evidence supplied by you, not automatic identification of the old binary. Set `shard_count` to the actual original business DB shard count. Target node IDs are 1–1023; replica counts cannot exceed the target node count. Physical hash slots are fixed at 256; `slot_count` sets logical Slots.
