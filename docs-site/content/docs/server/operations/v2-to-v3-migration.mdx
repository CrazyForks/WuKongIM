...
`source_commit` 当前必填，必须等于示例中的提交 SHA（原版 `v2.2.5-20260422`）。省略或填写其他值会被拒绝；它是你提供的来源版本证据，不会自动识别旧二进制。`shard_count` 填原部署实际业务 DB 分片数；目标节点 ID 为 1–1023，副本数不能超过目标节点数。`slot_count` 是目标逻辑 Slot 数，示例采用 v3 默认的 `12`，须与启动配置 `cluster.initial_slot_count` 一致；迁移计划仍须显式填写，不会自动补默认值。`hash_slot_count` 是物理哈希槽数，固定为 `256`。
...
复制成功后再执行校验命令；`-n` 只预演、不删除文件。要求退出码 0 且无差异，保存结果后再为实际服务账号设置访问权限。必须复制整个目录，包含 Controller 快照、迁移标记和隐藏文件。文件核对不能代替第 3 步的业务校验。
