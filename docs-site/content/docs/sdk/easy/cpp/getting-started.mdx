...
使用 C++17、CMake 和 WuKongEasySDK **WK_EASYSDK_CPP_VERSION** 完成在线收发。主流程采用 WuKongIM 维护的 vcpkg registry；其他安装方式见文末。
...
本文固定 vcpkg 工具版本为 `WK_EASYSDK_CPP_VCPKG_BASELINE`。
...
`WK_EASYSDK_CPP_SOURCE_REF`，与 registry baseline 分别固定。
...
希望跳过依赖编译时，从 [C++ SDK WK_EASYSDK_CPP_TAG Release](https://github.com/WuKongIM/WuKongEasySDK-CPP/releases/tag/WK_EASYSDK_CPP_TAG) 下载匹配的 ZIP 和 `SHA256SUMS`，验证 SHA-256 后解压。只需准备 CMake 3.20+ 和匹配的 C++ 开发环境，无需另装 vcpkg。
...
包名统一以 `WuKongEasySDK-CPP-WK_EASYSDK_CPP_VERSION-` 开头。包内包含 Debug/Release 静态 SDK、Boost/JSON 头文件、OpenSSL 库、许可文件和最小示例。在解压目录执行：
...
也可执行 `cmake --install build --config Release --prefix /path/to/sdk-prefix`，在下游用 `find_package(WuKongEasySDK WK_EASYSDK_CPP_CMAKE_VERSION CONFIG REQUIRED)`，并将 `CMAKE_PREFIX_PATH` 指向安装目录。静态库导出保留第三方依赖；Windows 分发需要携带所用的动态依赖。
