...
description: Install the pinned SDK version, obtain credentials from your backend, exchange online messages, and clean up.
---

This tutorial uses **WK_EASYSDK_ANDROID_VERSION** to exchange online messages between Alice and Bob in separate clients. WebSocket JSON-RPC CONNECT authenticates the connection.
...
```kotlin
dependencies {
    implementation("com.githubim:easysdk-android:WK_EASYSDK_ANDROID_VERSION")
}
```
...
Android EasySDK is a process singleton, and `WK_EASYSDK_ANDROID_TAG` cannot be initialized a second time with another configuration. This example fits a minimal run with one UID per process. A production application that switches accounts must first design and test a complete logout strategy around this limitation.
    ...
    fun connect(bootstrap: IMBootstrap) {
        ...
        if (current == null) {
            ...
            check(
                ...
                "WuKongEasySDK WK_EASYSDK_ANDROID_TAG cannot apply changed identity or configuration; " +
...
`onDestroy()` removes this page’s listeners and disconnects when the Activity finishes. An application connection belongs to the Application. Version `WK_EASYSDK_ANDROID_VERSION` cannot reinitialize with another identity or configuration; never silently reuse an old Token.

## 6. Troubleshooting

- **`SDK is not initialized`:** preserve the `init → addEventListener → connect` order.
- **The same UID gets a refreshed token or route and configuration changes:** `WK_EASYSDK_ANDROID_TAG` has no public reset API. Stop explicitly and restart the process; do not continue with a stale token or URL.
...
- **Raw JSON or `Params` still appears in Logcat with `debugLogging` disabled:** first confirm Gradle resolved `WK_EASYSDK_ANDROID_VERSION`, clean the old build, then use a non-production canary to distinguish SDK output from application or third-party network logs. Stop the rollout and file a minimal reproduction if the Release artifact still exposes that canary.
