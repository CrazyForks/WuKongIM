...
本教程使用 **WK_EASYSDK_IOS_VERSION**，让 Alice 与 Bob 在两个独立客户端中在线收发。连接由 WebSocket JSON-RPC CONNECT 完成鉴权。
...
Dependency Rule 选择 **Exact Version**，填写 `WK_EASYSDK_IOS_VERSION`。如果项目维护 `Package.swift`，使用精确规则：
