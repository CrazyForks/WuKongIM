...
This tutorial uses **WK_EASYSDK_IOS_VERSION** to exchange online messages between Alice and Bob in separate clients. WebSocket JSON-RPC CONNECT authenticates the connection.
...
Choose **Exact Version** and enter `WK_EASYSDK_IOS_VERSION`. If the project owns a `Package.swift`, use an exact rule:

```swift
dependencies: [
    .package(
        url: "https://github.com/WuKongIM/WuKongEasySDK-iOS.git",
        exact: "WK_EASYSDK_IOS_VERSION"
    )
]
```
...
```ruby
target 'YourApp' do
  pod 'WuKongEasySDK', 'WK_EASYSDK_IOS_VERSION'
end
```
